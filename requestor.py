...
class SimpleService(Service):
    ...
    async def start(self):
        """
        So the mongo container starts with a MongoDB instance running,
        so all the start function needs to do is verify that instance is 
    ...
    async def run(self):
        """
        Just forward endpoint requests to the instance via send_bytes
        nothing too big, Orchestration really is the main obstacle here
        also verify results or return errors via the endpoint. Perhaps
        ya-httpx is useful here?
        """
    ...
    async def shutdown(self):
        """
        Shutdown function is a little important here, so break this 
        process into 2 logical threads. The first handles requests like
        normal, the next begins backing up the database, then begins 
        transferring to another provider, ideally this occurs once a month
        at most
        """
